...
TOOL_TITLE = u"Применить тип марки по семейству"
...
try:
    ...
    if not target_tags:
        fail(
            u"Подходящие марки не найдены.\n\nКатегория: {}\nСемейство: {}\nТип элемента эталона: {}\nТип марки эталона: {}".format(
                ref_cat_name,
                ref_family_name,
                ref_type_name,
                ref_tag_type_name
            ),
            u"Применить тип марки по семейству"
        )

    updated = []
    skipped = []

    t = Transaction(doc, u"PP: Применить тип марки по семейству")
    t.Start()
    ...
    for tag in target_tags:
        try:
            ...
            if current_type_id == ref_tag_type_id:
                skipped.append(
                    u"Марка {}: тип уже совпадает".format(tag.Id.IntegerValue)
                )
                continue
