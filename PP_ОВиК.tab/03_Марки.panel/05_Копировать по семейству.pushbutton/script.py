...
def collect_candidate_elements_in_active_view(category_id, ref_family_name, ref_element_id):
    result = []
...
TOOL_TITLE = u"Копировать марки по семейству"
...
try:
    ...
    if not candidates:
        fail(
            ...
            u"Копировать марки по семейству"
        )

    created = []
    skipped = []

    t = Transaction(doc, u"PP: Копировать марки по семейству")
    t.Start()
