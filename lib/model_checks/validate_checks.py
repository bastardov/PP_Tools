# -*- coding: utf-8 -*-
"""Самопроверка пакета model_checks — запускать ОБЫЧНЫМ Python 3, Revit не нужен.

    python lib/model_checks/validate_checks.py

Что проверяет:
  1. синтаксис всех .py пакета и фасада lib/pp_model_checks.py;
...
def parse_all():
    ...
    facade = os.path.join(LIB, "pp_model_checks.py")
    if os.path.exists(facade):
        paths.append(facade)
    else:
        error(u"нет фасада lib/pp_model_checks.py")
    for path in paths:
        try:
    ...
    if func is None:
        ...
        if call_name(value) != "CheckDefinition":
            ...
            info["definition"] = {
                ...
                "option_keys": str_list(keyword(value, "option_keys")) or [],
            }
    return info
...
def stub_import(checks, options):
    ...
    try:
        import pp_model_checks as facade
    ...
    for name in ("create_element_cache", "get_check_definitions", "get_check_option_definitions",
                 "get_default_config", "run_report_checks"):
