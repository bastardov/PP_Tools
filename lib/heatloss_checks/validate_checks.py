# -*- coding: utf-8 -*-
"""Самопроверка пакета heatloss_checks («Проверка теплопотерь») — запускать ОБЫЧНЫМ Python 3, Revit не нужен.

    python lib/heatloss_checks/validate_checks.py

Что проверяет:
  1. синтаксис всех .py пакета и фасада lib/pp_heatloss_checks.py;
  ...
  3. у каждого check_*.py есть DEFAULTS, get_options(), get_definition(),
     а runner (функция или лямбда, которая её вызывает) определён в этом же файле;
  ...
  5. check_*.py не импортируют друг друга (общее — только через common*.py);
     обращения DEFAULTS["ключ"] в файле проверки указывают на ключ из
     DEFAULTS этого же файла;
...
FACADE = "pp_heatloss_checks.py"
FACADE_MODULE = "pp_heatloss_checks"
FACADE_NAMES = ("CheckCancelled", "create_run_context", "get_check_definitions",
    ...
    if func is None:
        ...
        if call_name(value) != "CheckDefinition":
            ...
            if isinstance(runner, ast.Lambda):
                # lambda doc, config, context: _run_..._check(doc, config, context)
                runner_name = call_name(runner.body) or None
...
class _Stub(object):
    ...
    def __int__(self):
        # pp_heatloss_spaces делает int(BuiltInCategory.OST_Walls) при импорте
        return abs(hash(self._path)) % 100000
